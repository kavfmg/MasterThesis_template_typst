...
このように引数に何もつけなければデフォルトで0.9em, 0.85emにする。level4以降については筆者は使っていないので設定しておらずそのままだと1emで表示される。このデフォルト値を変更したければ普通にtemplateの中でデフォルト値を書き換えるかsetすればいいだけだが、デフォルト値をいじった結果templateをimportしているすべてのファイルで形状が変わったらわけわからんのでmain冒頭で設定できるように、ということで引数として`dd`,`ddd`を受け取れるようにしている。それぞれdepth2,3の文字サイズを与える。
#grid(
  columns: 2,
  [
    ```typst
