...
このように引数に何もつけなければデフォルトで0.9em, 0.85emにする。level4以降については無視。これを変更したければ普通にtemplateの中でデフォルト値を書き換えるかsetすればいいだけだが、スペースの問題などで一時的にもう少し小さく、あるいは大きくしたいときもあるだろう、ということで、引数として`dd`,`ddd`を受け取るようにしている。それぞれdepth2,3の文字サイズを与える。
#grid(
  columns: 2,
  [
    ```typst
    #show list: smaller_list(dd: 1.2em, ddd: 0.3em)
    - タイトル
    - 本文
      - イントロ
        - 背景 
